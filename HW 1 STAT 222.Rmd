...
```
We can conclude that the treatment effects are not equal at the 95% confidence level since $0.05>p=0.01014922$. 
...
```
## Problem 3
The 95% confidence interval for a group mean for group k is denoted $\bar{y}_{k\cdot} \pm t_{n_{k}-1,\frac{\alpha}{2}} * \frac{s_{k}}{\sqrt{n_{k}}}$. Specifically, we can find $t_{n_{k}-1,\frac{\alpha}{2}}$ for the 95% confidence level and group $k=1$ using the R code below (where $n_{1}=7$ and $\frac{alpha}{2} = 0.025$):
```{r}
qt(1-0.025,6)
```
...
$\mu_{1} = 3.7457 \pm 2.446912 * \frac{0.2840}{\sqrt{7}} = 3.7457 \pm 0.2626562$, and so we can write that the 95% confidence interval for $\mu_{1}$ is $\mu_{1} \in (3.483044,4.008356)$.
...
:-----:|:--------:|:--------:|:--------:|:--------:
2-1  | -0.1657143  | 0.1052685  | -1.574205 | 0.06400576
3-1  | -0.147381  | 0.1131603  | -1.302409 | 0.102324
4-1  | 0.1767857  | 0.1052685  | 1.679379 | 0.9472324
3-2  | 0.01833333  | 0.1098475  | 0.166898 | 0.5656035
4-2  | 0.3425  | 0.1016991  | 3.367779 | 0.998772
4-3  | 0.3241667  | 0.1098475  | 2.95106 | 0.9966053
